	...
	while (n--) {
		if (playedWords.length > 0) {
			...
			for (let l = 0; l < word.tiles.length; ++l) {
				...
				if (stacked === 0) {
					resetBoardTile(tileHover, 60 * (word.tiles.length - l));
				} else {
...
const restoreProgress = (progress) => {
	...
	const totalPoints2 = wordsOnBoard.reduce((acc, w) => {
		...
		if (boardTiles.every(everyTileMatchesLetters(letters)) === false)
			return null;
...
const resetBoardTile = (tile, delay) => {
...
const onWordDrag = (word, mx, my) => {
	...
	let boardTile;
	let success = true;
	const overlappedTiles = []; // TODO(bret): Cache this on resize
	for (const tile of word.tiles) {
